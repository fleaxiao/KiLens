...
/** Adapt KiCad 10 name-only nets for the bundled KiCanvas parser, in memory only. */
export function normalizePreviewNets(text: string): string {
	const root = parseDocument(text);
	const board = root.items.find((item): item is List => item.kind === 'list' && head(item) === 'kicad_pcb');
	if (!board) return text;
	const numbers = new Map<string, number>();
	let nextNumber = 1;
	for (const item of board.items) {
		if (item.kind !== 'list' || head(item) !== 'net') continue;
		const number = secondAtom(item), name = item.items[2];
		if (number && name?.kind === 'atom' && Number.isFinite(Number(number.value))) {
			const code = Number(number.value);
			numbers.set(name.value, code);
			nextNumber = Math.max(nextNumber, code + 1);
		}
	}
	const replacements: TextReplacement[] = [];
	const definitions: string[] = numbers.has('') ? [] : ['(net 0 "")'];
	if (!numbers.has('')) numbers.set('', 0);
	function visit(list: List): void {
		for (const item of list.items) {
			if (item.kind !== 'list') continue;
			const name = secondAtom(item);
			if (head(item) === 'net' && item.items.length === 2 && name && text[name.start] === '"') {
				let number = numbers.get(name.value);
				const quoted = text.slice(name.start, name.end);
				if (number === undefined) {
					number = name.value ? nextNumber++ : 0;
					numbers.set(name.value, number);
					definitions.push(`(net ${number} ${quoted})`);
				}
				replacements.push({ start: item.start, end: item.end,
					text: `(net ${number}${head(list) === 'pad' ? ' ' + quoted : ''})` });
			} else visit(item);
		}
	}
	visit(board);
	if (!replacements.length) return text;
	// Definitions precede footprints for consumers that resolve references during parsing.
	const boardHead = board.items[0];
	replacements.push({ start: boardHead.end, end: boardHead.end, text: '\n' + definitions.join('\n') });
